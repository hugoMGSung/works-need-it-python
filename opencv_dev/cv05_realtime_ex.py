...
# 색감 변경 함수
def color_filter(img, color, scale):    # 입력값 영상, 변경할 색상, 변경할 비율
    dst = np.array(img, np.uint8)       # 입력 영상 복사
    if color == 'blue' or color == 0:
        dst[:, :, 0] = cv2.multiply(dst[:, :, 0], scale)    # 영상중 파란색 비율 변경
    elif color == 'green' or color == 1:
        dst[:, :, 1] = cv2.multiply(dst[:, :, 1], scale)    # 영상중 초록색 비율 변경
    if color == 'red' or color == 2:
        dst[:, :, 2] = cv2.multiply(dst[:, :, 2], scale)    # 영상중 빨간색 비율 변경
    return dst

# 밝기 변경 함수
def set_brightness(img, scale):         # 입력값 영상, 밝기를 변경할 값
    return cv2.add(img, scale)          # 전체 픽셀값에 scale을 더하여 밝기를 변경

# 대비 변경 함수
def set_contrast(img, scale):           # 입력값 영상, 대비를 변경할 값
